...
import static com.zielinski.kacper.inventoryapp.data.InventoryContract.*;
...
public class InventoryProvider extends ContentProvider {
    ...
    static {
        ...
        sUriMatcher.addURI(InventoryContract.CONTENT_AUTHORITY, PATH_INVENTORIES, INVENTORIES);
        sUriMatcher.addURI(InventoryContract.CONTENT_AUTHORITY, PATH_INVENTORIES + "/#", INVENTORY_ID);
    }
    ...
    private void validateData(ContentValues values) {
        ...
        String supplierPhoneNumber = values.getAsString(InventoryEntry.COLUMN_SUPPLIER_TELEPHONE_NUMBER);
        if (TextUtils.isEmpty(supplierPhoneNumber) || !validatePhoneNumber(supplierPhoneNumber)) {
            throw new IllegalArgumentException("Supplier telephone number is required.");
        }
    }
    ...
    public int update(Uri uri, ContentValues contentValues, String selection,
        ...
        switch (match) {
            case INVENTORIES:
                return updateInventories(uri, contentValues, selection, selectionArgs);
            case INVENTORY_ID:
                // For the INVENTORY_ID code, extract out the ID from the URI,
     ...
     */
    private int updateInventories(Uri uri, ContentValues values, String selection, String[] selectionArgs) {

        try {
            validateDataWithKeyCheck(values);
        } catch (IllegalArgumentException e) {
            Toast.makeText(getContext(), e.getMessage(), Toast.LENGTH_SHORT).show();
            return 0;
        }
    ...
    private void validateDataWithKeyCheck(ContentValues values) {
        ...
        if(values.containsKey(InventoryEntry.COLUMN_SUPPLIER_TELEPHONE_NUMBER)) {
            String supplierPhoneNumber = values.getAsString(InventoryEntry.COLUMN_SUPPLIER_TELEPHONE_NUMBER);
            if (TextUtils.isEmpty(supplierPhoneNumber) || !validatePhoneNumber(supplierPhoneNumber)) {
                throw new IllegalArgumentException("Supplier telephone number is required.");
            }
